"""Paired accumulated CPU-C1 comparison: frozen CPU-R1 versus CPU-R6."""
...
FROZEN_LIBRARY = ROOT / "native_cpu" / "validation" / "cpu-r1" / "frozen" / "minimind_cpu.dll"
FROZEN_IDENTITY = ROOT / "native_cpu" / "validation" / "cpu-r1" / "frozen" / "identity.json"
CANDIDATE_LIBRARY = ROOT / "native_cpu" / "build" / "Release" / "minimind_cpu.dll"
CANDIDATE_BUILD_INFO = ROOT / "native_cpu" / "benchmarks" / "cpu-r6" / "build-info.json"
PAIR_COUNT = 6
REFERENCE_RUNTIME_COMMIT = "a6d06a949cb514741f100e439e283d2189685102"
CANDIDATE_RUNTIME_COMMIT = "ae90d82e6007574628d496eba898cdffce2b8096"
...
def identity():
    ...
    return {
        ...
        "candidate_build_info_sha256": sha(CANDIDATE_BUILD_INFO) if CANDIDATE_BUILD_INFO.is_file() else None,
        "policy": {"temperature": 0.9, "top_k": 50, "top_p": 0.85, "seed": 0, "system": "official MiniMind chat system"},
        "threads": 1,
        "cpus": [0],
        "profiler": False,
        "candidate_options": {"selective_logits": True, "reuse_kv": True, "v_blocked_attention": True, "gqa_k_shared": True, "gqa_v_shared": True, "ffn_f16_storage": True, "ffn_row4": False},
        "reference_options": {"native_defaults": True, "selective_logits": False, "reuse_kv": False, "v_blocked_attention": False, "gqa_k_shared": False, "gqa_v_shared": False, "ffn_f16_storage": False, "ffn_row4": False},
...
def run_entry(benchmark, profile, workload, *, length=None):
    ...
    with NativeRuntime(MODEL, profile["library"], 2048, threads=1, cpus=[0], row_weights=[1]) as runtime:
        ...
        data["configured_options"]["native_defaults"] = options.get("native_defaults", False)
        data["effective"] = {
...
def make_cases():
    ...
    for workload in ("historical", "second"):
        cases.append({"key": f"cpu-c1-conversation-{workload}", "workload": workload})
    for length in (256, 1792):
        cases.append({"key": f"cpu-c1-forced-{length}", "workload": "forced", "length": length})
    return cases
...
def main(argv=None):
    ...
    meta = identity()
    profiles = {
        "A": {"name": "A", "library": FROZEN_LIBRARY, "library_sha256": meta["reference_library_sha256"], "options": meta["reference_options"]},
        "B": {"name": "B", "library": CANDIDATE_LIBRARY, "library_sha256": meta["candidate_library_sha256"], "options": meta["candidate_options"]},
    }
    summaries = []
