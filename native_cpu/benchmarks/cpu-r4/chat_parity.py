"""Run the historical six-turn conversation against the CPU-R4 profiles."""
...
def _load_benchmark():
    ...
    spec = importlib.util.spec_from_file_location("cpu_r1_benchmark_for_cpu_r4_chat", path)
...
def main() -> int:
    ...
    profiles = {name: _run(benchmark, name, tokenizer) for name in ("K0", "K1")}
    comparison = benchmark.compare_entries(profiles["K0"], profiles["K1"], "historical")
    checks = []
    for index, (left, right) in enumerate(
        zip(profiles["K0"]["turns"], profiles["K1"]["turns"]), start=1
    ):
        checks.append(
            {
                "turn": index,
    ...
    result = {
        ...
        "reference": profiles["K0"],
        "candidate": profiles["K1"],
    }
    OUTPUT.write_text(
