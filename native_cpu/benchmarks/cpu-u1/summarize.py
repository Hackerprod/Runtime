"""Render the CPU-U1 consolidation comparison and fail closed on invalid receipts."""
...
def evaluate(summary: dict[str, Any]):
    ...
    if set(cases) != set(REQUIRED_CASES):
        reasons.append("summary case set does not match the four-case CPU-U1 protocol")
    ...
    if identity.get("cpus") != [0] or identity.get("threads") != 1 or identity.get("profiler") is not False:
        reasons.append("CPU-U1 participant or profiler policy is not recorded correctly")
