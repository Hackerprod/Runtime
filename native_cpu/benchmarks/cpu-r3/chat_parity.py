"""Run the historical six-turn conversation against the CPU-R3 profiles."""
...
def _load_benchmark():
    ...
    spec = importlib.util.spec_from_file_location("cpu_r1_benchmark_for_cpu_r3_chat", path)
...
def _run(benchmark, profile_name: str, tokenizer):
    ...
    with NativeRuntime(
        ...
        cpus=config["cpus"],
        row_weights=config["row_weights"],
    ) as runtime:
        ...
        data, session = benchmark.run_conversation(
            runtime,
            tokenizer,
            config,
            benchmark.HISTORICAL_PROMPTS,
            256,
        )
...
def main() -> int:
    ...
    profiles = {name: _run(benchmark, name, tokenizer) for name in ("F0", "F1")}
    comparison = benchmark.compare_entries(profiles["F0"], profiles["F1"], "historical")
    checks = []
    for index, (left, right) in enumerate(
        zip(profiles["F0"]["turns"], profiles["F1"]["turns"]), start=1
    ):
        checks.append(
            {
                "turn": index,
    ...
    result = {
        ...
        "profiles": {
            name: data["effective"] for name, data in profiles.items()
        },
        "checks": checks,
        "comparison": comparison,
        "reference": profiles["F0"],
        "candidate": profiles["F1"],
    }
    OUTPUT.write_text(
