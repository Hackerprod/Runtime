...
def build_session(args):
    ...
    try:
        ...
        if args.backend == "original" and (threads != 1 or cpus is not None or row_weights is not None or diagnostics):
        ...
        args.reuse_kv = args.backend == "native"
        ...
        session = ChatSession(counted, tokenizer, context_limit=args.context_limit, max_new_tokens=args.max_new_tokens,
                              ...
                              top_k=getattr(args, "top_k", 0), top_p=getattr(args, "top_p", 1.0), reuse_kv=(args.backend == "native"))
...
def response_metrics(args, result, counted, loading_seconds):
    ...
    return {"backend": args.backend, "profile": args.profile, "system": args.system, "runtime_backend": result.backend,
            ...
            "attention_f16_storage": bool(getattr(args, "attention_f16_storage", False)),
            "ffn_f16_storage_bytes": getattr(counted, "ffn_f16_storage_bytes", None),
