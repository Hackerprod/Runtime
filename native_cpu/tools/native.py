...
class NativeRuntime:
    ...
    def _bind(self):
        ...
        self._query_lm_head_f16_storage = getattr(lib, "mm_lm_head_f16_storage", None)
        self._query_lm_head_f16_storage_bytes = getattr(lib, "mm_get_lm_head_f16_storage_bytes", None)
        self._query_lm_head_f16_prepare_ns = getattr(lib, "mm_get_lm_head_f16_prepare_ns", None)
        ...
        if self._query_ffn_f16_storage_bytes is not None:
            self._query_ffn_f16_storage_bytes.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint64)]
            self._query_ffn_f16_storage_bytes.restype = ctypes.c_int
        ...
        if self._query_lm_head_f16_storage is not None:
            self._query_lm_head_f16_storage.argtypes = [ctypes.c_void_p]; self._query_lm_head_f16_storage.restype = ctypes.c_int
        if self._query_lm_head_f16_storage_bytes is not None:
            self._query_lm_head_f16_storage_bytes.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint64)]
            self._query_lm_head_f16_storage_bytes.restype = ctypes.c_int
        if self._query_lm_head_f16_prepare_ns is not None:
            self._query_lm_head_f16_prepare_ns.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint64)]
            self._query_lm_head_f16_prepare_ns.restype = ctypes.c_int
    ...
    @property
    def ffn_f16_storage_bytes(self):
        self._check()
        if self._query_ffn_f16_storage_bytes is None: return None
        value = ctypes.c_uint64()
        if int(self._query_ffn_f16_storage_bytes(self._handle, ctypes.byref(value))) != 0: return None
        return int(value.value)

    @property
    def ffn_f16_prepare_ns(self):
        self._check()
    ...
    @property
    def lm_head_f16_storage(self):
        self._check()
        if self._query_lm_head_f16_storage is None: return False
        return bool(self._query_lm_head_f16_storage(self._handle))

    @property
    def lm_head_f16_storage_bytes(self):
        self._check()
        if self._query_lm_head_f16_storage_bytes is None: return None
        value = ctypes.c_uint64()
        if int(self._query_lm_head_f16_storage_bytes(self._handle, ctypes.byref(value))) != 0: return None
        return int(value.value)

    @property
    def lm_head_f16_prepare_ns(self):
        self._check()
        if self._query_lm_head_f16_prepare_ns is None: return None
        value = ctypes.c_uint64()
        if int(self._query_lm_head_f16_prepare_ns(self._handle, ctypes.byref(value))) != 0: return None
        return int(value.value)
