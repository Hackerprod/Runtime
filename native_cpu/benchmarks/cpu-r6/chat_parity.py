"""Run the historical six-turn conversation against CPU-R6 X0/X1."""
...
def _load_benchmark():
    ...
    spec = importlib.util.spec_from_file_location("cpu_r1_benchmark_for_cpu_r6_chat", path)
...
def _run(benchmark, profile_name: str, tokenizer):
    ...
    with NativeRuntime(MODEL, LIBRARY, 2048, threads=config["threads"], cpus=config["cpus"], row_weights=config["row_weights"]) as runtime:
        ...
        runtime.configure_ffn_f16_storage(config["ffn_f16_storage"])
...
def main() -> int:
    ...
    profiles = {name: _run(benchmark, name, tokenizer) for name in ("X0", "X1")}
    comparison = benchmark.compare_entries(profiles["X0"], profiles["X1"], "historical")
    checks = []
    for index, (left, right) in enumerate(zip(profiles["X0"]["turns"], profiles["X1"]["turns"]), start=1):
        checks.append({
            "turn": index,
    ...
    result = {
        ...
        "reference": profiles["X0"],
        "candidate": profiles["X1"],
    }
